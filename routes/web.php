...
Route::middleware(['auth', 'operator'])->prefix('admin')->name('admin.')->group(function () {
    ...
    // Throttled like `packages.retention.preview` below, and for the same reason: aborting
...
Route::middleware(['auth', 'super'])->prefix('admin')->name('admin.')->group(function () {
    ...
    // Rotating a secret must not cost the provider's SSO identities — see rotateSecret().
    Route::patch('oidc/{provider}/secret', [Admin\OidcProviderController::class, 'rotateSecret'])->name('oidc.secret');
