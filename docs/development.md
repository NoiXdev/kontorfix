...
- **Verify and pin the image.** The release workflow signs every published image by digest
  ...
  the provenance attestation and SBOM it already produced. `docker/compose.yaml` runs that same
  image: its four service references used to name a Harbor path the release pipeline never
  populated — scaffolding from before that pipeline existed — so the header's verification
  instructions and the `image:` lines named different registries and the signature covered
  nothing the stack pulled. Both sides say `docker.io` now, and `KONTORFIX_IMAGE` is the one
  knob to override them with a verified `@sha256:<digest>`. Provenance says how an image was
