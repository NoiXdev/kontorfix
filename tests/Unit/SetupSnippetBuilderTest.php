...
it('builds composer, auth and npm snippets for a slug-based registry', function () {
    ...
    expect($snips['npm'])->toContain('@<dein-scope>:registry=https://reg.example.test/r/kunde/acme/')
        ->and($snips['npm'])->not->toContain("\nregistry=")
        ->toContain('//reg.example.test/r/kunde/acme/:_authToken=<dein-token>');
});
...
it('builds pip and twine snippets for the Python registry', function () {
    ...
    expect($snips['pip'])
        ...
        ->toContain('https://token:<dein-token>@reg.example.test/r/kunde/acme/simple/')
