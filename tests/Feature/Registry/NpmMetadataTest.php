...
it('serves an npm packument for an assigned scoped package', function () {
    ...
    $response = $this->withHeaders(tokenHeaderFor($group))
        ->getJson(registryPath($group).'/@noixdev/ui-kit')
        ->assertOk()
    ...
    expect($response->json('versions')['1.0.0']['dist']['tarball'])
        ->toBe('http://localhost'.registryPath($group).'/@noixdev/ui-kit/-/ui-kit-1.0.0.tgz');
});
...
it('serves an unscoped packument', function () {
    ...
    $this->withHeaders(tokenHeaderFor($group))->getJson(registryPath($group).'/leftpad')->assertOk()->assertJsonPath('name', 'leftpad');
});
...
it('does not shadow the composer root or p2 routes', function () {
    $group = Group::factory()->for(Organization::factory())->create(['slug' => 'kadenz']);
    $this->withHeaders(tokenHeaderFor($group))->getJson(registryPath($group).'/packages.json')->assertOk()
...
it('does not expose a composer package via the npm packument endpoint (cross-type isolation)', function () {
    ...
    $this->withHeaders(tokenHeaderFor($group))->getJson(registryPath($group).'/shared-name')->assertNotFound();
});
...
it('routes case-variant of packages.json as an npm package name, not the composer root', function () {
    ...
    $this->withHeaders(tokenHeaderFor($group))->getJson(registryPath($group).'/Packages.json')->assertNotFound();
});
