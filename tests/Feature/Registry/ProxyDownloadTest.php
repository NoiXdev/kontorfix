...
it('downloads a composer artifact from the upstream, caches it and streams it', function () {
    ...
    Http::fake();
    $this->withHeaders(tokenHeaderFor($group))->get(registryPath($group)."/proxy/composer/{$up->id}/acme/demo/1.0.0.0")->assertOk();
...
it('downloads an npm tarball from the upstream and streams octet-stream', function () {
    ...
    $this->withHeaders(tokenHeaderFor($group))
        ->get(registryPath($group)."/proxy/npm/{$up->id}/left-pad/-/left-pad-1.0.0.tgz")
...
it('401 without token, 404 for an upstream of another group', function () {
    ...
    $this->get(registryPath($group)."/proxy/composer/{$otherUp->id}/acme/demo/1.0.0.0")->assertUnauthorized();
    $this->withHeaders(tokenHeaderFor($group))
        ->get(registryPath($group)."/proxy/composer/{$otherUp->id}/acme/demo/1.0.0.0")->assertNotFound();
});
...
it('404 when the requested version is not in the cached metadata', function () {
    ...
    $this->withHeaders(tokenHeaderFor($group))
        ->get(registryPath($group)."/proxy/composer/{$up->id}/acme/demo/9.9.9.0")->assertNotFound();
});
...
it('enforces strict mode on the download route even with cached metadata', function () {
    ...
    $this->withHeaders(tokenHeaderFor($group))
        ->get(registryPath($group)."/proxy/composer/{$up->id}/evil/pkg/1.0.0.0")->assertNotFound();
    Http::assertNothingSent();

    // Once allowlisted: downloadable.
    $up->allowedPackages()->create(['name' => 'evil/pkg']);
    $this->withHeaders(tokenHeaderFor($group))
        ->get(registryPath($group)."/proxy/composer/{$up->id}/evil/pkg/1.0.0.0")->assertOk();
});
...
it('refuses a hostile upstream dist url pointing at an internal address', function () {
    ...
    $this->withHeaders(tokenHeaderFor($group))
        ->get(registryPath($group)."/proxy/composer/{$up->id}/acme/demo/1.0.0.0")->assertStatus(422);
...
it('refuses an upstream redirect to a host that resolves to an internal address', function () {
    ...
    $this->withHeaders(tokenHeaderFor($group))
        ->get(registryPath($group)."/proxy/composer/{$up->id}/evil/pkg/1.0.0.0")->assertStatus(502);
...
it('404s a download through a disabled upstream', function () {
    ...
    $this->withHeaders(tokenHeaderFor($group))
        ->get(registryPath($group)."/proxy/composer/{$up->id}/acme/demo/1.0.0.0")->assertNotFound();
});
